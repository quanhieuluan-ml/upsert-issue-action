...
async function collectChanges(
  ...
  do {
    ...
    allChangedFiles.push(
      ...fileList.data.map((file: { filename: any }) => file.filename)
    )
...
async function findIssue(title: string): Promise<any> {
  ...
  if (issues.length > 1) {
    core.warning(
      'Found multiple issues for title ' +
        title +
        '. Use the latest one \n' +
        JSON.stringify(issues, null, 2)
    )
    issue = issues.reduce((prev: any, current: any) => {
      return new Date(prev.createTime) > new Date(current.createTime)
        ? prev
        : current
    })
  } else {
...
async function listAllIssues(endpoint: string, title: string) {
  ...
  async function fetchPage(
    ...
    let filtered = data.issues.filter(
...
async function updateIssuePlan(
  issue: any,
  changes: Change[],
  title: string
): Promise<any> {
  ...
  core.info(
    'Check existing plan for update:\n' + JSON.stringify(planData, null, 2)
  )
  ...
  for (const change of changes) {
    ...
    for (const step of planData.steps) {
      ...
      if (!matchedSpec) {
        throw new Error(
          `Bytebase disallow adding new migration file to the existing issue: ${change.file}`
        )
      }
    }
  }
  ...
  for (const step of planData.steps) {
    for (const spec of step.specs) {
      ...
      if (!matchedSpec) {
        throw new Error(
          'Bytebase disallow removing migration file from the existing issue.'
        )
      }
    }
  }
      ...
      for (const change of changes) {
        if (
          ...
          const sheetRes = await fetch(
            `${projectUrl}/sheets/${sheetUid}?${queryParams}`,
            {
              method: 'GET',
              headers
            }
              ...
              for (const task of stage.tasks) {
                if (change.id == task.specId) {
                  if (!allowedStates.includes(task.status)) {
                    throw new Error(
                      `Can not update migration file: ${change.file}. Task status ${task.status} not in [${allowedStates.toString()}].`
                    )
                  }
                }
              }
  ...
  if (updatePlan) {
    ...
    core.info(
      'Skip plan update. No migration file changed since the last time.'
    )
  }
}
...
async function createRollout(planName: string): Promise<any> {
  try {
    ...
    core.setFailed(
      error instanceof Error ? error.message : 'Failed to create rollout'
    )
  }
  return {}
}
